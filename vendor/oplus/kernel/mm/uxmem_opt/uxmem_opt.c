...
static void pageset_update(void *data, unsigned long *high, unsigned long *batch)
{
	*high *= 2;
}
...
static int register_uxmem_opt_vendor_hooks(void)
{
	...
	ret = register_trace_android_vh_pageset_update(pageset_update, NULL);
	if (ret != 0) {
		pr_err("register_trace_android_vh_pageset_update failed! ret=%d\n", ret);
		goto out;
	}
...
static void unregister_uxmem_opt_vendor_hooks(void)
{
	...
	unregister_trace_android_vh_pageset_update(pageset_update, NULL);
