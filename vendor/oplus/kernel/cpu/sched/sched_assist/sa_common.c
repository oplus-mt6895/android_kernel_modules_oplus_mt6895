...
unsigned int ux_task_exec_limit(struct task_struct *p)
{
	 ...
	 */
	im_flag = oplus_get_im_flag(p);
	if (test_bit(IM_FLAG_SS_LOCK_OWNER, &im_flag) &&
...
void cgroup_set_sched_assist_boost_task(struct task_struct *p,  char *comm)
{
	...
	if (unlikely(cgroup_check_set_sched_assist_boost(comm))) {
		...
		if (is_top(p) || test_task_ux(p->group_leader)) {
			...
			oplus_set_im_flag(p, IM_FLAG_LIGHT_THREAD);
		} else {
			oplus_set_ux_state_lock(p, (ux_state & ~SA_TYPE_HEAVY), true);
...
void sched_assist_target_comm(struct task_struct *task, const char *buf)
{
	...
	strlcpy(comm, buf, TASK_COMM_LEN);
	...
	if (im_mali(comm)) {
		oplus_set_ux_state_lock(task, SA_TYPE_LIGHT, true);
	}
#endif
...
void android_rvh_dequeue_task_handler(void *unused, struct rq *rq, struct task_struct *p, int flags)
{
	...
	if ((flags & DEQUEUE_SLEEP)) {
		...
		if (ots && p->state & TASK_UNINTERRUPTIBLE)
...
static inline void do_boost_kill_task(struct task_struct *p)
{
	...
	set_user_nice(p, -20);
	if (!is_32bit) {
		cpumask_copy(&p->cpus_mask, cpu_possible_mask);
		p->nr_cpus_allowed = cpumask_weight(cpu_possible_mask);
	}
}
...
void register_profile_event(void)
{
	/* register a notifier to monitor task exit */
	profile_event_register(PROFILE_TASK_EXIT, &process_exit_notifier_block);
}
