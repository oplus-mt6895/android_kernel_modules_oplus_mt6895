...
static int im_flag_set_handle(struct task_struct *task, int im_flag)
{
	...
	if (((im_flag == (IM_FLAG_SS_LOCK_OWNER + IM_FLAG_CLEAR)) && (test_bit(IM_FLAG_SS_LOCK_OWNER, &old_im)))||
		((im_flag == IM_FLAG_SS_LOCK_OWNER) && (!test_bit(IM_FLAG_SS_LOCK_OWNER, &old_im))))
