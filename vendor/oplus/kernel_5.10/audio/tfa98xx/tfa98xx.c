...
static enum Tfa98xx_Error tfa9874_calibrate(struct tfa98xx *tfa98xx_cal, int *speakerImpedance, bool b_aging)
{
	...
	if (tfa98xx->tfa->dev_idx == 0) {
		...
		if (tfa98xx_ins2 != NULL) {
			pr_info("ins2 tfaContWriteProfile\n");
		...
		if ((tfa98xx_device_count == 2)||(tfa98xx_device_count == 4)) {
			...
			list_for_each_entry(tfa98xx, &tfa98xx_device_list, list) {
				if (tfa98xx->tfa->channel != 0xff){
					...
					#ifdef OPLUS_ARCH_EXTENDS
					/*if channel not initialized, set as defalut, cnt-id 0,2 is left, cnt-id 1,3 is right*/
		...
		*speakerImpedance = tfa_get_calibration_info(tfa98xx->tfa, 0);
		...
		pr_info("%s: devid %d 0x%x Calibration value: %d.%03d ohm.\n", __func__, tfa98xx->tfa->dev_idx, tfa98xx_cal->i2c->addr, *speakerImpedance/1000, *speakerImpedance%1000);
				...
				loff_t *ppos)
{
	...
	if (tfa98xx->tfa->in_use == 0){
		pr_debug("%s: tfa is not open!\n",__func__);
		return -ENODEV;
	}
...
/* kcontrol for channel select */
static int g_tfa_chan_sel = 0;
static int g_tfa_chan_send = 0;

static int tfa98xx_adsp_send_chan_sel(struct tfa98xx *tfa98xx, int chan_sel)
{
    uint8_t bytes[9] = {0};
    int ret = 0;
	...
	mutex_lock(&tfa98xx->dsp_lock);
#ifdef CONFIG_MTK_PLATFORM
	ret = tfa98xx_send_data_to_dsp(&bytes[0], sizeof(bytes));
#else
	ret = send_tfa_cal_in_band(&bytes[0], sizeof(bytes));
#endif
	mutex_unlock(&tfa98xx->dsp_lock);
	msleep(5);

	if (ret)
		pr_err("send chan_sel 0x%02x%02x to adsp failed. ret = %d\n", bytes[4], bytes[5], ret);
	else
		pr_info("send chan_sel 0x%02x%02x to adsp sucess.\n", bytes[4], bytes[5]);
	return ret;
}

static int tfa98xx_info_chan_sel_ctl(struct snd_kcontrol *kcontrol,
                                struct snd_ctl_elem_info *uinfo)
{
	uinfo->type = SNDRV_CTL_ELEM_TYPE_INTEGER;
	uinfo->count = 1;
...
static int tfa98xx_set_chan_sel_ctl(struct snd_kcontrol *kcontrol,
                               struct snd_ctl_elem_value *ucontrol)

{
	...
	chan_sel = ucontrol->value.integer.value[0];
	pr_info("%s:set selector from 0x%04x to 0x%04x\n", __func__, g_tfa_chan_sel, chan_sel);
	g_tfa_chan_sel = chan_sel;

	if ((tfa98xx->tfa->is_probus_device) && (tfa98xx->dsp_init == TFA98XX_DSP_INIT_DONE) ) {
		tfa98xx_adsp_send_chan_sel(tfa98xx, g_tfa_chan_sel);
	}
...
static int tfa98xx_get_chan_sel_ctl(struct snd_kcontrol *kcontrol,
                               struct snd_ctl_elem_value *ucontrol)
{
	mutex_lock(&tfa98xx_mutex);

	ucontrol->value.integer.value[0] = g_tfa_chan_sel;
...
/*  end kcontrol for channel select */
...
/*  Add for TFA_CHAN_SELECTOR */
	...
	tfa98xx_controls[mix_index].name = "TFA_CHAN_SELECTOR";
	tfa98xx_controls[mix_index].iface = SNDRV_CTL_ELEM_IFACE_MIXER;
	tfa98xx_controls[mix_index].info = tfa98xx_info_chan_sel_ctl;
	tfa98xx_controls[mix_index].get = tfa98xx_get_chan_sel_ctl;
	tfa98xx_controls[mix_index].put = tfa98xx_set_chan_sel_ctl;
	mix_index++;
...
static int tfa98xx_load_container(struct tfa98xx *tfa98xx)
{
	const struct firmware *pFirmware = NULL;
	int rc = 0;
	const char *new_fw_name = (tfa98xx->tfa->fw_name == NULL) ? fw_name : tfa98xx->tfa->fw_name;
...
enum Tfa98xx_Error tfa98xx_adsp_send_calib_values(void)
{
	...
	list_for_each_entry(tfa98xx, &tfa98xx_device_list, list) {
		...
		if (TFA_GET_BF(tfa, MTPEX) == 1) {
			...
			pr_info("Device 0x%x cal value is 0x%x(%d.%03d ohm)\n",
	...
	if (tfa98xx_device_count == bytes[0]) {
		...
		if (tfa98xx_device_count == 4) {
			...
			bytes_id2[2] = 0x81;
			bytes_id2[3] = 0x05;
			ret = tfa98xx_send_data_to_dsp(&bytes_id2[1], sizeof(bytes_id2)-1);
			if (ret) {
#if IS_ENABLED(CONFIG_OPLUS_FEATURE_MM_FEEDBACK)
				fb_err |= 0x04;
#endif /*CONFIG_OPLUS_FEATURE_MM_FEEDBACK*/
...
static int tfa98xx_mute(struct snd_soc_dai *dai, int mute, int stream)
{
	...
	if (mute) {
		...
		if (stream == SNDRV_PCM_STREAM_PLAYBACK) {
			tfa98xx->pstream = 1;
			if (tfa98xx->tfa->is_probus_device){
				if (g_tfa_chan_send == 0) { // only send once per playback.
					tfa98xx_adsp_send_chan_sel(tfa98xx, g_tfa_chan_sel);
					g_tfa_chan_send = 1;
				}
				...
				const struct i2c_device_id *id)
{
	...
	if (gpio_is_valid(tfa98xx->reset_gpio)) {
		...
		if (ret) {
			pr_err("reset gpio %d request failed\n", tfa98xx->reset_gpio);
			return ret;
		}
	}
